...
class Kuka:
  ...
  def reset(self):
    ...
    p.resetBasePositionAndOrientation(self.kukaUid,[-0.09,-0.01000,0.000000],[0.000000,0.000000,0.000000,1.000000])
    self.jointPositions=[ 0.006418, 0.413184, -0.011401, -1.589317, 0.005379, 1.137684, -0.006539, 0.000048, -0.299912, 0.000000, -0.000043, 0.299960, 0.000000, -0.000200 ]
    self.numJoints = p.getNumJoints(self.kukaUid)
    for jointIndex in range (self.numJoints):
      p.resetJointState(self.kukaUid,jointIndex,self.jointPositions[jointIndex])
      p.setJointMotorControl2(self.kukaUid,jointIndex,p.POSITION_CONTROL,targetPosition=self.jointPositions[jointIndex],force=self.maxForce)
    self.endEffectorPos = [0.5,0.0,0.6]
  ...
  def applyAction(self, motorCommands,terminate):
    ...
    if (self.useInverseKinematics):
      ...
      if (self.endEffectorPos[0]<0.2):
        self.endEffectorPos[0]=0.2
      self.endEffectorPos[1] = self.endEffectorPos[1]+dy
      if (self.endEffectorPos[1]<-0.5):
        self.endEffectorPos[1]=-0.5
      if (self.endEffectorPos[1]>0.5):
        self.endEffectorPos[1]=0.5
      
    
      self.endEffectorPos[2] = self.endEffectorPos[2]+dz
      if (self.endEffectorPos[2]<0.25):
      ...
      if (self.useSimulation):
          ...
          p.setJointMotorControlArray(self.kukaUid, range(7), p.POSITION_CONTROL,
			targetPositions=jointPoses[0:7], targetVelocities=[0.0]*7,
			positionGains=[0.3] * 7, velocityGains=[1] * 7)
      else:
