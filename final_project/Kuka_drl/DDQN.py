...
class DoubleDQN:
    ...
    def learn(self):
        ...
        q_next, q_evalNext = self.sess.run(
            [self.q_next, self.q_eval],
            feed_dict={
                self.s_: batch_memory[:, -self.state_shape:],  # next obs
                self.s: batch_memory[:, -self.state_shape:],  # next obs
            })
        q_eval = self.sess.run(self.q_eval,{self.s: batch_memory[:, :self.state_shape]})
        # change q_target w.r.t q_eval's action
        q_target = q_eval.copy()

        batch_index = np.arange(self.batch_size, dtype=np.int32)
        eval_act_index = batch_memory[:, self.state_shape].astype(int)
        reward = batch_memory[:, self.state_shape + 1]
        if self.double_q:
            max_act4next = np.argmax(q_evalNext,axis=1)
        ...
        _, self.cost = self.sess.run([self._train_op, self.loss],
                                     feed_dict={self.s: batch_memory[:, :self.state_shape],
